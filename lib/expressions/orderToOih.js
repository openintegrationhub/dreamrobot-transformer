...
module.exports.getExpression = msg => {
  ...
  const expression = {
    ...
    invoiceNumber:            jsonata(`$number("${msg.body.order.invoice_number}")`).evaluate(),
    paidAmount:               jsonata(`$number("${msg.body.order.paid_amount}")`).evaluate(),
    ...
    shipping:
      {
        ...
        address: {
          ...
          streetNumber:       msg.body.order.shipping.address.street === null ? undefined : jsonata(`$number($trim($split("${ msg.body.order.shipping.address.street}", " ")[-1]))`).evaluate(),
    ...
    customer: {
      ...
      addresses:[{
          ...
          streetNumber:       msg.body.order.customer.address.street === null ? undefined : jsonata(`$number($trim($split("${ msg.body.order.customer.address.street}", " ")[-1]))`).evaluate(),
