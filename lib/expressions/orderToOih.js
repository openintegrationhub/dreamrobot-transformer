...
module.exports.getExpression = msg => {
  ...
  const expression = {
    ...
    shipping:
      {
        ...
        address: {
          ...
          streetNumber:       2,//msg.body.order.shipping.address.street === null ? undefined : jsonata(`$trim($split("${ msg.body.order.shipping.address.street}", " ")[-1])`).evaluate(),
    ...
    lineItems:                jsonata(`each("${msg.body.order.line}", function($index) {
   		{
        	"name": "${msg.body.order.line}[$index].name",
            "sku":  "${msg.body.order.line}[$index].sku,
            "quantity": "${msg.body.order.line}[$index].quantity,
            "price":         "${msg.body.order.line}[$index].price,
            "taxAmount":     "${msg.body.order.line}[$index].tax,
            "productId":     "${msg.body.order.line}[$index].transaction_id,
            "references": [
              {
                "externalId":               "${msg.body.order.line}"[$index].stock_align_id,
    ...
    customer: {
      ...
      addresses:[{
          ...
          streetNumber:       2,//msg.body.order.customer.address.street === null ? undefined : jsonata(`$trim($split("${ msg.body.order.customer.address.street}", " ")[-1])`).evaluate(),
